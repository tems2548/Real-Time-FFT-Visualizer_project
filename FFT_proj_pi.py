...
# ----------------- Settings -----------------
PORT = "COM6"   # change to match your serial port
...
fig.canvas.manager.set_window_title("Real-Time FFT Analyzer with Filter Toggle & CSV Save")
...
def find_peaks(volt, fs):
    volt = volt - np.mean(volt)
    if filter_enabled:
        volt = bandpass_filter(volt, fs)
...
while True:
    try:
        ...
        adc_vals = np.frombuffer(frame[2:2+N*2], dtype=np.uint16)
        FS = struct.unpack('f', frame[-4:])[0]

        # --- Voltage Conversion ---
        volt = (adc_vals / ADC_MAX) * VREF
        volt = volt - np.mean(volt)
        volt = volt * 1.05  # small amplitude correction

        freq_axis, mag_db, main_freq, main_amp, sec_freq, sec_amp, rms, snr_db = find_peaks(volt, FS)
        ...
        sec_marker.set_data([sec_freq], [20*np.log10(sec_amp)])
        ...
        if now - prev_time >= 1.0:
            ...
            frame_count = 0
            prev_time = now
            fig.suptitle(f"Main: {main_freq:.1f} Hz | 2nd: {sec_freq:.1f} Hz | RMS: {rms:.3f} V | FS={FS:.1f} Hz | FPS: {fps_display:.1f}")
        plt.pause(0.001)
